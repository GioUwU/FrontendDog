...
export function Creacion() {
  ...
  const handleSubmit = async e => {
    ...
    if (state.nombre === '' || state.temperamento === '' || state.pesoMin === '' || state.pesoMax === '' || state.alturaMin === '' || state.alturaMax === '' || state.vidaMin === '' || state.vidaMax === '') {
      setState({
        ...state,
        noValidation: true
      })
      ...
      <div className='form'>
        <form onSubmit={handleSubmit}>
          ...
          <div className='formItem'>
            ...
            <input className='shorterImput' type="number" min="0" onChange={handleChange} value={state.pesoMax} name='pesoMax' placeholder='max' />
            &nbsp;<span className='aligned'>*</span>
          </div>
          <div className='formItem'>
            <span>Weight(cm):</span>&nbsp;
